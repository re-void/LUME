...
/**
 * Конвертирует Ed25519 публичный ключ в X25519 публичный ключ.
 * ПРИМЕЧАНИЕ: tweetnacl не поддерживает Ed25519→X25519 конвертацию нативно.
 * В проекте используются отдельные пары ключей (signing + exchange),
 * поэтому эта функция не применяется и оставлена как заглушка.
 * @deprecated Используйте отдельные exchange-ключи вместо конвертации.
 */
export function ed25519ToX25519PublicKey(ed25519PublicKey: Uint8Array): Uint8Array {
    console.warn('ed25519ToX25519PublicKey is a no-op stub. Use separate exchange keys.');
 ...
 */
export function verify(message: Uint8Array, signature: Uint8Array, publicKey: string): boolean {
...
export function encrypt(
    ...
    const ciphertext = nacl.box(message, nonce, recipientPubKeyBytes, senderSecKeyBytes);
...
export function decrypt(
    ...
    return nacl.box.open(ciphertextBytes, nonceBytes, senderPubKeyBytes, recipientSecKeyBytes);
}
