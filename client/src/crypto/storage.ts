 ...
 */
async function decryptWithPin(encrypted: EncryptedDataV1, pin: string): Promise<string | null> {
...
let cachedMasterKey: { pin: string; saltB64: string; key: Uint8Array } | null = null;
...
async function getMasterKey(pin: string): Promise<Uint8Array> {
    ...
    if (cachedMasterKey && cachedMasterKey.pin === pin && cachedMasterKey.saltB64 === saltB64) {
...
async function encryptForStorage(data: string, pin: string): Promise<EncryptedDataV2> {
...
async function decryptFromStorage(encrypted: EncryptedData, pin: string): Promise<string | null> {
 ...
 */
export async function saveIdentityKeys(keys: IdentityKeys, pin: string): Promise<void> {
 ...
 */
export async function loadIdentityKeys(pin: string): Promise<IdentityKeys | null> {
...
/**
 * Проверяет правильность PIN
 */
export async function verifyPin(pin: string): Promise<boolean> {
    const storedHash = await get<string>(STORAGE_KEYS.PIN_HASH);
    ...
    const pinBytes = stringToUint8Array(pin);
    const inputHash = encodeBase64(nacl.hash(pinBytes));
 ...
 */
export async function saveContacts(contacts: Contact[], pin: string): Promise<void> {
...
export async function savePreKeyMaterial(material: LocalPreKeyMaterial, pin: string): Promise<void> {
    const data = JSON.stringify(material);
    const encrypted = await encryptForStorage(data, pin);
    await set(STORAGE_KEYS.PREKEYS, encrypted);
}

export async function loadPreKeyMaterial(pin: string): Promise<LocalPreKeyMaterial | null> {
    ...
    try {
        ...
        if (!material.signedPreKey || !Array.isArray(material.oneTimePreKeys)) return null;
        return material;
    } catch {
...
export async function consumeOneTimePreKey(publicKey: string, pin: string): Promise<KeyPair | null> {
    const material = await loadPreKeyMaterial(pin);
    if (!material) return null;

    const index = material.oneTimePreKeys.findIndex((k) => k.publicKey === publicKey);
...
export async function saveRatchetSessions(sessions: RatchetSessions, pin: string): Promise<void> {
    const data = JSON.stringify(sessions);
    const encrypted = await encryptForStorage(data, pin);
    await set(STORAGE_KEYS.SESSIONS, encrypted);
}

export async function loadRatchetSessions(pin: string): Promise<RatchetSessions> {
...
export async function deleteRatchetSession(contactId: string, pin: string): Promise<void> {
 ...
 */
export async function deleteContact(contactId: string, pin: string): Promise<void> {
 ...
 */
export async function importEncryptedBackup(encoded: string, pin: string): Promise<void> {
    let envelope: BackupEnvelope;
    try {
        const json = uint8ArrayToString(decodeBase64(encoded));
        envelope = JSON.parse(json) as BackupEnvelope;
        if (envelope.v !== 1 || !envelope.salt || !envelope.nonce || !envelope.ciphertext) {
